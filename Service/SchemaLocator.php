...
class SchemaLocator
{
        ...
        if (is_dir($dir = $bundle->getPath().'/Resources/config')) {
            ...
            if (iterator_count($schemas)) {
                foreach ($schemas as $schema) {
                    try {
                        $resourcesFilename = $this->transformToResourcesFilename($schema, $bundle);
                        $finalSchema = new \SplFileInfo($this->fileLocator->locate($resourcesFilename));
                    }
                    catch (\Throwable $e) {
    ...
    protected function transformToResourcesFilename(\SplFileInfo $schema, BundleInterface $bundle)
    {
        $schemaPath = str_replace(
            $bundle->getPath(). DIRECTORY_SEPARATOR . 'Resources' . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR,
            '',
        ...
        return sprintf('%s/src/Resources/%s/config/%s', $this->configuration['paths']['projectDir'], $bundle->getName(), $schemaPath);
    }
}
